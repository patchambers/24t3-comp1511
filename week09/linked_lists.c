...
// List ordered insert
// Inserts a new node into a linked list such that the list remains in 
// ascending order
//
// Parameters:
//      head: A pointer to the head of the linked list in ascending order
//      data: The data to be stored in the new node
//
// Returns:
//      A pointer to the head of the linked list
...
int main(void) {
    ...
    // Insert 12 into the list, keeping it in ascending order
    head = list_ordered_insert(head, 12);
...
struct node *list_ordered_insert(struct node *head, int data) {
    ...
    // If data is smaller than data in first node
    if (head->data > data) {
        new->next = head;
        return new;
    }
