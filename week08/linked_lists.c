...
struct node *insert_head(struct node *head, int data) {
    ...
    // Return the new head of the list (this is just the new node we created)
    return new;
}
...
struct node *insert_tail(struct node *head, int data) {
    ...
    // Loop until the last node in the list
    struct node *current = head;
    while (current->next != NULL) {
        current = current->next;
    }

    // Current now points at the last node
    current->next = new;
...
int list_length(struct node *head) {
    struct node *current = head;
    int count = 0;
    // Loop through list and increment count each iteration
    while (current != NULL) {
        count++;
        current = current->next;
    }
